...
def map_sfc_to_grid(
    ...
    for i in range(sfc_grid_data.shape[0]):
        for j in range(sfc_grid_data.shape[1]):
            lon = (
                gridlon[i, j]
                if gridlon[i, j] >= 0
            ...
            sfc_grid_data[i, j] = sfc_data[i2, j2]
...
def set_sfcemis(
    ...
    if iemslw == 1:
        ...
        for i in range(sfcemis.shape[0]):
            for j in range(sfcemis.shape[1]):
                if islmsk[i, j] == 1:
